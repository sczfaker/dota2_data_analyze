...
class Match_stat(object):
    ...
    def __init__(self,update_dataset_and_testset=False):
        ...
        if update_dataset_and_testset:
            ...
            pre_df2=pd.read_csv("matches_testset_0727.csv")
            self.pre_lenth=len(pre_df1.append(pre_df2))
            self.MODEL_NAME='model_'+str(datetime.date.today())+"_"+str(self.pre_lenth)+'.pkl'#"model_2020-07-06.pkl#"model_2020-07-06.pkl"#MODEL_NAME="model_2020-07-10_801.pkl"
    ...
    def match_team(self):
        return 
    def model_rank(self):
        real_test_set=pd.read_csv("matches_list_ranking_0707.csv")
        for i in real_test_set:3
update_model_logic=True
show_image=True
update_model_more=False
SELECT_MODEL=""
TESTMODEL_PERFORM=False
REALTIME_PREDICT=False
if update_model_more==True:
    pass
if update_model_logic==True:
    print ("start..")
    instance=Match_stat(update_dataset_and_testset=True)#update_dataset_and_testset=True
    train_result=instance.train()
    print (train_result,"训练成功:","模型(逻辑回归):")
if REALTIME_PREDICT==True:
    instance=Match_stat(update_dataset_and_testset=False)#update_dataset_and_testset=True
